...
var Pagination = {
    ...
    Last: function () {
        Pagination.code += '<i aria-label=" list continues "></i><button role="link" data-text="' + Pagination.size + '"  class="by-keyboard"><span class="show-for-sr">page </span>' + Pagination.size + '</button>';
    },

    // add first page with separator
    First: function () {
        Pagination.code += '<button role="link" data-text="1" class="by-keyboard"><span class="show-for-sr">page </span>1</button><i aria-label="..."></i>';
    },
    ...
    Start: function () {
        ...
        if (Pagination.pageLanguage === 'fr') {
            resulText = `${fromRecord} à ${toRecord} de ${Pagination.totalResults} résultats`;
    ...
    Create: function (e) {
        ...
        var html = [
            ``,
            `<button role="link" id="prevButton" class="prev-button by-keyboard"><em class="show-for-sr">${prevPage} </em><img alt="" src="img/icons/chevron-left.svg"/></button>`, // previous button
            '<span class="desktop"></span>',  // pagination container
            `<input onclick="this.select()" aria-describedby="mobile-aria-pages" type="number" pattern="[0-9]*" inputmode="numeric" class="paginationTextField" id="mobilePageShow" value="${Pagination.page}">`,
            `<span id="mobile-aria-pages" class="mobileText">${textForMobile}</span>`, // Mobile pagination
            `<button role="link" id="nextButton" class="next-button by-keyboard"><em class="show-for-sr">${nextPage} </em><img alt="" src="img/icons/chevron-right.svg"></button>`,  // next button
            ``
        ];
